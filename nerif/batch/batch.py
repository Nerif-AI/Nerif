...
class Batch:
    ...
    @classmethod
    def list(cls, after: Optional[str] = None, limit: int = 20) -> Dict[str, Any]:
        """
        ...
        sorted_batches = sorted(cls._batches.values(), key=lambda b: b["created_at"], reverse=True)

        # Apply pagination
        if after:
            try:
                after_idx = next(i for i, b in enumerate(sorted_batches) if b["id"] == after)
    ...
    async def _process_batch(self, batch_id: str, requests: List[Dict[str, Any]], endpoint: str) -> None:
        """
        ...
        try:
            ...
            batch["errors"] = {"object": "list", "data": [{"code": "batch_processing_error", "message": str(e)}]}
            self._save_batch_state(batch_id, batch)
            LOGGER.error(f"Batch {batch_id} failed: {e}")

    async def _process_chat_batch(
        self, batch_id: str, requests: List[Dict[str, Any]], results: List[Dict[str, Any]], errors: List[Dict[str, Any]]
    ) -> None:
    ...
    def _process_single_chat(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model = SimpleChatModel(model=model_name, temperature=temperature, max_tokens=max_tokens)

        # Format messages for the model
        for msg in messages:
            model.messages.append({"role": msg["role"], "content": msg["content"]})
    ...
    def _create_error_response(self, request: Dict[str, Any], exception: Exception) -> Dict[str, Any]:
        """Create an error response for a failed request."""
        return {
            "custom_id": request["custom_id"],
            "response": {"status_code": 500, "request_id": f"req_{uuid.uuid4().hex[:24]}", "body": None},
            "error": {"message": str(exception), "type": type(exception).__name__, "code": "internal_error"},
        }

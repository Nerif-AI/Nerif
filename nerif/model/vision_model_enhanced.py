...
class VisionModelWithCompression:
    ...
    def _get_vlm_optimized_settings(self) -> dict:
        ...
        if "gpt-4" in self.model.lower():
            ...
            # Default settings
            return {"max_size": (2048, 2048), "quality": 85, "max_file_size_mb": 10}
        ...
        try:
            ...
            if image.mode == "RGBA" or (image.mode == "P" and "transparency" in image.info):
                ...
                for quality in [target_quality, target_quality - 10, target_quality - 20]:
                    ...
                    LOGGER.info(
                        f"Compressed JPEG (quality {quality}): {len(image_data)} -> {len(compressed_data)} bytes ({compressed_size_mb:.2f}MB)"
                    )
    ...
    def add_image(self, image_source: Union[str, Path, bytes], source_type: str = "auto"):
        """

...
""",
    ...
    parser.add_argument("images", nargs="+", type=str, help="Image file(s) to compress")

    parser.add_argument("-o", "--output", type=str, help="Output path (file for single image, directory for multiple)")

    parser.add_argument("-t", "--threshold", type=float, default=1.0, help="Size threshold in MB (default: 1.0)")

    parser.add_argument("--jpeg-quality", type=int, default=85, help="JPEG compression quality 1-100 (default: 85)")

    parser.add_argument("--png-level", type=int, default=9, help="PNG compression level 0-9 (default: 9)")

    parser.add_argument("--no-preserve-metadata", action="store_true", help="Do not preserve EXIF metadata")

    parser.add_argument(
        "--convert-threshold", type=float, default=0.7, help="PNG to JPEG conversion threshold (default: 0.7)"
    )

    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed compression information")

    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be compressed without actually doing it"
    )
    ...
    if single_file_mode:
        ...
        if not args.dry_run:
            try:
                was_compressed, ratio, message = compressor.compress_image(
                    image_path, output_path, preserve_metadata=not args.no_preserve_metadata
                )
        ...
        if not args.dry_run:
            results = compressor.compress_batch(image_paths, output_dir, preserve_structure=True)
